...
const emptyForm = { username: '', password: '', employee_id: '', name: '', email: '', mobile_number: '', class_id: '', designation: '' }
...
export default function HODFaculty() {
  ...
  const load = () => {
    setLoading(true)
    Promise.all([api.get('/users/faculty'), api.get('/classes/')]).then(([f, c]) => {
      setFaculty(f.data); setClasses(c.data); setLoading(false)
  ...
  const del = async (id) => {
    ...
    try {
      ...
      load()
    } catch (err) { alert(err.response?.data?.detail || 'Error deleting faculty member') }
  }
  ...
  return (
    <div>
      <PageHeader 
        title="Faculty" 
        subtitle={`${faculty.length} faculty members in your department`}
