...
export default function AdminEvents() {
  ...
  const [modal, setModal] = useState(null) // null | 'create' | event
  ...
  const targetColor = (type) => ({
    COLLEGE: 'purple', DEPARTMENT: 'blue', CLASS: 'green'
  })[type] || 'gray'
              ...
              <div key={e.id} className="px-5 py-4 flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-sm font-medium text-gray-900 dark:text-white truncate">{e.title}</h3>
                    {e.targets?.map((t, i) => (
                      <Badge key={i} color={targetColor(t.target_type)}>{t.target_type}</Badge>
                    ))}
                  </div>
                  <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
                    <span>{e.event_date}</span>
                    {e.start_time && (
                      <span className="flex items-center gap-1"><Clock size={11} />{e.start_time}</span>
                    )}
                    {e.venue && (
                      <span className="flex items-center gap-1"><MapPin size={11} />{e.venue}</span>
                    )}
                    {e.creator_name && <span>by {e.creator_name}</span>}
                    {e.attachment_url && (
