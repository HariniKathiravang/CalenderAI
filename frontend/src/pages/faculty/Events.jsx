...
export default function FacultyEvents() {
  ...
  const handleCreate = async (payload) => { await api.post('/events/', payload); setModal(null); load() }
  const handleUpdate = async (payload) => { await api.put(`/events/${modal.id}`, payload); setModal(null); load() }
  ...
  const filtered = myEvents.filter(e => e.title.toLowerCase().includes(search.toLowerCase()))
  const targetColor = (type) => ({ COLLEGE: 'purple', DEPARTMENT: 'blue', CLASS: 'green' })[type] || 'gray'

  return (
    <div>
      <PageHeader title="Class Events" subtitle="Events you've created"
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
        ...
        {loading ? (
          ...
          <p className="text-sm text-gray-400 text-center py-12">No events created yet</p>
        ) : (
